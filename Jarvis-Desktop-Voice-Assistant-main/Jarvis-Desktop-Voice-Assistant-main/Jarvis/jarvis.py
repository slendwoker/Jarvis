...
import datetime
import speech_recognition as sr #Для распознования речи 
import wikipedia #Для поиска инфы в Wiki
import webbrowser as wb #Открытие web
import os
import random
import pyautogui
import pyaudio
...
def speak(audio) -> None: #Данная функция озвучивает текст 
    engine.say(audio)
    engine.runAndWait()
...
def wishme() -> None: # Приветствует пользователя в зависимости от времени суток 
...
def takecommand(): #Функция, которая распознает голосовую команду пользователя через микрофон
    r = sr.Recognizer()
...
if __name__ == "__main__":
    wishme()
    while True: #Цикл, по которому он отвечает на заготовленные фразы
        ...
        elif "offline" in query: #При команде ofline завершает работу
            quit()
